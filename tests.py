...
class TestConstraints(unittest.TestCase):
    ...
    def test_discovery(self) -> None:
        """Test data type"""
        self.assertEqual(
                self.data.get_data_type(d1.data, "stroke"), 'bool'
                )
        self.assertEqual(
                self.data.is_nullable(d1.data, "gender"), False
                )
        self.assertEqual(
                self.data.is_unique(d1.data, "bmi"), False
                )
        self.assertEqual(
                self.data.max_length(d1.data, "Residence_type"), 5
                )
        self.assertEqual(
                self.data.min_length(d1.data, "Residence_type"), 5
                )
        self.assertEqual(
