    ...
    def _guess_date_types(self) -> pd.DataFrame:
        ...
        for date in date_cols:
            self.data[date] = pd.to_datetime(
                self.data[date], errors="ignore", infer_datetime_format=True
    ...
    def _optimize_dtypes(self) -> pd.DataFrame:
        ...
        for col in self.data.columns:
            if issubclass(self.data[col].dtypes.type, np.int_):
                ...
                self.data[col] = pd.to_numeric(
                    self.data[col], downcast="float"
                )
            elif issubclass(self.data[col].dtypes.type, np.object_) and (
                self.data[col].duplicated().any()
            ):
